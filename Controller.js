        ...
        try {
            ...
            if (status == 'down') {
                ...
                const [insert] = await DB.execute(
                    "INSERT INTO `his_dfr` (`alat`, `ip`, `tanggal_down`, `waktu_down`) VALUES (?,?,?,?)",
                    [alat,ip,tanggal,time] 
                );
                const [update] = await DB.execute(
                    "UPDATE `real_dfr` SET `status` = ? where `ip` = ?",
                    [status,ip]
                );
                ...
                if(down_time_key && tanggal_down){
                    ...
                    if(monthDifference === 0){
                        ...
                        const [update] = await DB.execute(
                            "UPDATE `his_dfr` SET `tanggal_up` = ?, `waktu_up`=?, `durasi_down`=? where `alat`=? and `ip`=? and `tanggal_down`=? and `waktu_down`=?",
                    ...
                    const [tableupdate1] = await DB.execute(
                        "UPDATE `real_DFR` SET `link_status` = ? WHERE `ip` = ?",
                        [status, ip]
                    );
                    results.push({
                        operation: "update1",
                    ...
                    for (var i = 1; i <= monthDifference; i++) {
                        ...
                        if (i === 1) {
                            const [result1] = await DB.execute(
                                "UPDATE `his_DFR` SET  `Tanggal_Up`=?, `Waktu_Up`=?  WHERE `Tanggal_Down`=? AND `Waktu_Down`=?  and `ip` = ?",
                        ...
                        const [insertupdate] = await DB.execute(
                            "INSERT INTO `his_DFR` (`alat`, `ip`, `Tanggal_Down`, `Waktu_Down`, `Tanggal_Up`, `Waktu_Up`) VALUES (?,?,?,?,?,?)",
